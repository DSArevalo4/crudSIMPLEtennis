        ...
        <div class="partido-card ${estado === 'finalizado' ? 'partido-finalizado' : ''}">
            ...
            ${(user.perfil === 'administrador' || user.perfil === 'profesor') ? `
                <div class="partido-card-footer">
                    ...
                    ` : ''}
                    <button class="btn-sm btn-secondary" onclick="openPartidoForm(${partido.id})">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
let currentPartidoEdit = null;

async function openPartidoForm(partidoId = null) {
    try {
        const modal = document.getElementById('partidoFormModal');
        const title = document.getElementById('partidoFormTitle');
        const form = document.getElementById('partidoForm');
        
        // Reset form
        form.reset();
        currentPartidoEdit = partidoId;
        
        // Cargar datos para los selects
        await Promise.all([
            loadTorneosForSelect(),
            loadDeportistasForSelect()
        ]);
        
        if (partidoId) {
            title.textContent = 'Editar Partido';
            await loadPartidoData(partidoId);
        } else {
...
function closePartidoFormModal() {
    const modal = document.getElementById('partidoFormModal');
    modal.style.display = 'none';
    currentPartidoEdit = null;
}
...
async function loadTorneosForSelect() {
    try {
        ...
        const select = document.getElementById('partido-torneo');
        
        select.innerHTML = '<option value="">Seleccione un torneo</option>';
        torneos.forEach(torneo => {
            const option = document.createElement('option');
            option.value = torneo.id;
            option.textContent = `${torneo.nombre} (${torneo.superficie})`;
...
async function loadDeportistasForSelect() {
    try {
        const response = await api.get('/api/usuarios');
        if (!response.ok) throw new Error('Error al cargar deportistas');
        
        const usuarios = await response.json();
        const deportistas = usuarios.filter(u => u.perfil === 'deportista' && u.activo);
        
        const select1 = document.getElementById('partido-deportista1');
        const select2 = document.getElementById('partido-deportista2');
        
        const optionsHtml = '<option value="">Seleccione deportista</option>' + 
            deportistas.map(d => `<option value="${d.id}">${d.nombre} ${d.apellido}</option>`).join('');
        
        select1.innerHTML = optionsHtml;
        select2.innerHTML = optionsHtml.replace('Seleccione deportista', 'Seleccione deportista 2');
    } catch (error) {
        console.error('Error cargando deportistas:', error);
        showNotification('Error al cargar deportistas', 'error');
    }
}

async function loadPartidoData(partidoId) {
    try {
        const partido = partidos.find(p => p.id === partidoId);
        if (!partido) {
        ...
        document.getElementById('partido-torneo').value = partido.torneo_id || '';
        document.getElementById('partido-deportista1').value = partido.deportista1_id || '';
        document.getElementById('partido-deportista2').value = partido.deportista2_id || '';
        document.getElementById('partido-ronda').value = partido.ronda || '';
        document.getElementById('partido-fecha').value = partido.fecha_partido || '';
        document.getElementById('partido-numero-ronda').value = partido.numero_ronda || '';
        document.getElementById('partido-posicion').value = partido.posicion_cuadro || '';
        document.getElementById('partido-estado').value = partido.estado || 'programado';
    } catch (error) {
...
async function savePartido() {
    try {
        ...
        const data = {
            torneo_id: parseInt(formData.get('torneo_id')),
            deportista1_id: parseInt(formData.get('deportista1_id')),
            deportista2_id: parseInt(formData.get('deportista2_id')),
            ronda: formData.get('ronda') || null,
            fecha_partido: formData.get('fecha_partido') || null,
            numero_ronda: formData.get('numero_ronda') ? parseInt(formData.get('numero_ronda')) : null,
            posicion_cuadro: formData.get('posicion_cuadro') ? parseInt(formData.get('posicion_cuadro')) : null,
            estado: formData.get('estado') || 'programado'
        };
        
        // Validar que los deportistas sean diferentes
        if (data.deportista1_id === data.deportista2_id) {
            showNotification('Los deportistas deben ser diferentes', 'error');
            return;
        }
        
        let response;
        if (currentPartidoEdit) {
            // Actualizar partido existente
            response = await api.put(`/api/partidos/${currentPartidoEdit}`, data);
        } else {
            // Crear nuevo partido
            response = await api.post('/api/partidos', data);
        }
        
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.error || 'Error al guardar partido');
        }
        ...
        showNotification(
            currentPartidoEdit ? 'Partido actualizado exitosamente' : 'Partido creado exitosamente',
            'success'
        );
        
        closePartidoFormModal();
        await loadPartidos();
    } catch (error) {
